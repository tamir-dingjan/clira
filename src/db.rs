...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        // check if the file path exists, and propagate any errors
        let file = File::open(&self.file_path)?;
        let reader = BufReader::new(file);

        // read the file path into a buffer, propagate any errors
        let db: DBState = serde_json::from_reader(reader)?;
    ...
    fn write_db(&self, db_state: &DBState) -> Result<()> {
        ...
        serde_json::to_writer(writer, db_state)?;
